...
def doublefifo(clock, reset, dfifo_bus, buffer_sel, depth=16):
    ...
    def logic():
        ...
        if buffer_sel:
            dfifo_bus.read_data.next = fbus1.read_data
            fbus1.read.next = dfifo_bus.read
            dfifo_bus.empty.next = fbus1.empty

        else:
            dfifo_bus.read_data.next = fbus2.read_data
            fbus2.read.next = dfifo_bus.read
            dfifo_bus.empty.next = fbus2.empty
