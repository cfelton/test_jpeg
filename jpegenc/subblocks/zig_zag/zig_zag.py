...
import myhdl
from myhdl import Signal, intbv, always_seq, block
...
def zig_zag(inputs, outputs, clock, reset, N=8):
    ...
    ior = inputs.out_range
    output_sigs = [Signal(intbv(0, min =-ior, max=ior))
                   for _ in range(N**2)]
    input_sigs = [Signal(intbv(0, min =-ior, max=ior))
                  for _ in range(N**2)]
